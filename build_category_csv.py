...
DEFAULT_OUTPUT_ROOT_PARTS = ("data",)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Фильтрует CSV-файлы data* из downloads и раскладывает строки по "
            "downloads/data/<group>/v2 по спискам категорий."
        )
    )
    parser.add_argument(
        "--downloads-dir",
        ...
        help="Папка с исходными CSV-файлами data*.",
    )
    parser.add_argument(
        "--categories-module",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Корень для выходных папок. По умолчанию используется downloads/data, "
            "то есть результат пойдёт в downloads/data/<group>/<version>."
        ),
    )
    parser.add_argument(
        "--reset-output",
        action="store_true",
        help="Удалить ранее сгенерированные CSV-файлы в выходных папках перед запуском.",
    )
...
def find_input_files(downloads_dir: Path) -> list[Path]:
    ...
    if not files:
        raise FileNotFoundError(
            f"В {downloads_dir} не найдено входных CSV-файлов вида data*."
        )

    return files


def resolve_output_root(downloads_dir: Path, output_root: Path | None) -> Path:
    if output_root is not None:
        return output_root.resolve()
    return downloads_dir.joinpath(*DEFAULT_OUTPUT_ROOT_PARTS)


def reset_output_dirs(output_root: Path, version: str) -> None:
    for group_name in GROUP_NAMES:
        target_dir = output_root / group_name / version
...
def detect_csv_delimiter(source_path: Path) -> str:
    sample = source_path.read_text(encoding="utf-8-sig", errors="ignore")[:8192]
    if not sample.strip():
        return ","
...
def read_csv_file(source_path: Path) -> tuple[pd.DataFrame, str]:
    delimiter = detect_csv_delimiter(source_path)
    df = pd.read_csv(source_path, sep=delimiter, encoding="utf-8-sig", low_memory=False)
    return df, delimiter
...
def build_output_path(
    ...
    return output_root / group_name / version / source_path.name
...
def process_file(
    ...
    df, delimiter = read_csv_file(source_path)
    ...
    for group_name in GROUP_NAMES:
        output_path = build_output_path(output_root, group_name, version, source_path)
        ...
        group_df.to_csv(output_path, index=False, sep=delimiter, encoding="utf-8-sig")
        stats.rows_per_group[group_name] += len(group_df)


def print_summary(output_root: Path, version: str, stats: RunStats) -> None:
...
def main() -> None:
    ...
    output_root = resolve_output_root(downloads_dir, args.output_root)
    ...
    if args.reset_output:
        reset_output_dirs(output_root, args.version)

    stats = RunStats()
    for source_path in input_files:
        process_file(source_path, output_root, args.version, category_lookup, stats)

    print_summary(output_root, args.version, stats)
